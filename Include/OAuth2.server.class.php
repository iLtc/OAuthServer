 ...
 * Feel free to change.
 */
define("OAUTH2_CLIENT_ID_REGEXP", "/^[0-9]{1,32}$/i");
...
abstract class OAuth2 {
   ...
   */
  abstract protected function setAccessToken($oauth_token, $client_id, $expires, $scope = NULL, $uid);
   ...
   * @ingroup oauth2_section_4
   */
  protected function setAuthCode($code, $client_id, $redirect_uri, $expires, $scope = NULL, $uid) {
  }
  ...
  public function finishClientAuthorization($is_authorized, $params = array()) {
    ...
    if ($is_authorized === FALSE) {
      ...
      if ($response_type == OAUTH2_AUTH_RESPONSE_TYPE_AUTH_CODE || $response_type == OAUTH2_AUTH_RESPONSE_TYPE_CODE_AND_TOKEN)
        $result["query"]["code"] = $this->createAuthCode($client_id, $redirect_uri, $scope, $uid);

      if ($response_type == OAUTH2_AUTH_RESPONSE_TYPE_ACCESS_TOKEN || $response_type == OAUTH2_AUTH_RESPONSE_TYPE_CODE_AND_TOKEN)
        $result["fragment"] = $this->createAccessToken($client_id, $scope, $uid);
    }
   ...
   * @ingroup oauth2_section_3
   */
  private function createAuthCode($client_id, $redirect_uri, $scope = NULL, $uid) {
    $code = $this->genAuthCode();
    $this->setAuthCode($code, $client_id, $redirect_uri, time() + $this->getVariable('auth_code_lifetime', OAUTH2_DEFAULT_AUTH_CODE_LIFETIME), $scope, $uid);
    return $code;
  }
